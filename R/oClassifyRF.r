...
#' @note none
#' @export
#' @include oClassifyRF.r
#' @examples
#' \dontrun{
#' sClass <- oClassifyRF(df_predictor, GSP, GSN)
#' }

oClassifyRF <- function(df_predictor, GSP, GSN, nfold=3, nrepeat=10, seed=825, mtry=NULL, ntree=1000, cv.aggregateBy=c("logistic","Ztransform","fishers","orderStatistic","none"), rangeMax=5, verbose=TRUE, ...)
{
    ...
    if(!is.data.frame(df_predictor) | !is.matrix(df_predictor)){
		df_predictor <- df_predictor %>% as.data.frame()
    }else{
	...
	if(nfold==1 & nrepeat==1){
		...
		ls_model <- lapply(1:length(index_sets), function(i){
			...
			if(verbose){
				message(sprintf("\tRepeatFold %d: %d GSP + %d GSN (%s) ...", i, table(trainset$class)[2], table(trainset$class)[1], as.character(Sys.time())), appendLF=TRUE)
			}
	...
	ls_res <- lapply(1:length(lsls_predictors), function(i){
		...
		ls_pPerf <- lapply(ls_predictors, function(x){
			pPerf <- oClassifyPerf(prediction=x, GSP=GSP, GSN=GSN, verbose=FALSE)
		})
		
		# do plotting
		bp <- oClassifyComp(ls_pPerf)
	...
	if(!is.null(df_res)){
		## df_auroc
		df_res <- as.matrix(oSparseMatrix(df_res[,c('methods','model','auroc')], verbose=FALSE))
		...
		## df_fmax
		df_res <- do.call(rbind, ls_res)
		df_res <- as.matrix(oSparseMatrix(df_res[,c('methods','model','fmax')], verbose=FALSE))
		ind <- match(c("Supervised",colnames(df_predictor)), rownames(df_res))
		if(nfold==1 & nrepeat==1){
			df_res <- as.matrix(df_res[ind,], ncol=nfold)
			colnames(df_res) <- 'Fold1'
	...
	if(cv.aggregateBy=="none"){
		...
		## priority: being rescaled into the [0,rangeMax] range
		priority <- vec_full
		priority <- rangeMax * (priority - min(priority))/(max(priority) - min(priority))
		...
		df_full <- as.matrix(oSparseMatrix(df_full, verbose=FALSE))
		...
		## priority: first log10-transformed ap and then being rescaled into the [0,rangeMax] range
